...
public class OrderController {
	...
	public ModelAndView createOrder(@ModelAttribute("createOrderForm") Order order, BindingResult result,HttpServletRequest request) {

		ModelAndView modelAndView = new ModelAndView();

		if(request.getParameter("items") != null){
			Item item = new Item();
			item.setItemId(Integer.parseInt(request.getParameter("items")));
		...
		if(result.hasErrors()||order.getItems().isEmpty()) {
//			modelAndView.setViewName("orders/orders-createOrder");
			return new ModelAndView("redirect:/orders/create?customerId="+order.getCustomer().getCustomerId());
		} else {
			if(orderService.addOrder(order)) {
				modelAndView.setViewName("redirect:/customer/"+order.getCustomer().getCustomerId());
			}
