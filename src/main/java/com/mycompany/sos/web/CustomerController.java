...
public class CustomerController  {
	...
	public ModelAndView createCustomer(
		...
		if(customerAdded != null) {
			...
			modelAndView.setViewName("redirect:/customer/" + customer.getCustomerId());
		} else {
	...
	/**
	 * Deleting customer and related orders 
	 *
	 * @param customerId id identifying the customer
	 * @param model Spring's framework model object
	 * @return view name
	 */
	@RequestMapping(value = "customers/delete/{customerId}", method = RequestMethod.GET)
	public ModelAndView deleteCustomer(@PathVariable("customerId") String customerId, Model model) {
	...
	public ModelAndView editCustomer(@PathVariable("customerId") String customerId) {
		ModelAndView modelAndView = new ModelAndView("customers/customers-createCustomer");
		Customer customer = customerService.findCustomerByCustomerId(Integer.parseInt(customerId));
		modelAndView.addObject("createCustomerForm", customer);
		return modelAndView;
	}
}
